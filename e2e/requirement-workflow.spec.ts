...
test('projects setup validates agent team then starts requirement workflow', async ({ page }) => {
  ...
  await page.route('**/api/v1/projects/demo/workflow-runs/current', async (route) => route.fulfill({ status: 404, json: { code: 'WORKFLOW_RUN_NOT_FOUND', message: 'Project has no workflow run yet' } }))
  ...
  await page.route('**/api/v1/projects/demo/agent-configuration/contexts', async (route) => route.fulfill({ json: [] }))
...
test('project with an existing run opens its workspace directly', async ({ page }) => {
  const run = { id: 'run-7', projectId: 'test', status: 'RUNNING', stages: [] }
  await page.route('**/api/v1/auth/session', async (route) => route.fulfill({ json: { accessToken: 'fixture', user: { id: 1, displayName: 'Fixture', permissions: ['skill:browse'] } } }))
  await page.route('**/api/v1/auth/feishu/document-access', async (route) => route.fulfill({ json: { status: 'NOT_AVAILABLE' } }))
  await page.route(/\/api\/v1\/projects(\?.*)?$/, async (route) => route.fulfill({ json: { items: [{ projectKey: 'test', name: 'Test', role: 'OWNER', status: 'ACTIVE' }] } }))
  await page.route('**/api/v1/projects/test/members', async (route) => route.fulfill({ json: [{ userId: 1, displayName: 'Fixture', role: 'OWNER', status: 'ACTIVE' }] }))
  await page.route('**/api/v1/projects/test/workflow-runs/current', async (route) => route.fulfill({ json: run }))
  await page.route('**/api/v1/workflow-runs/run-7', async (route) => route.fulfill({ json: run }))
  await page.route('**/api/v1/workflow-runs/run-7/stages', async (route) => route.fulfill({ json: [] }))
  await page.route('**/api/v1/workflow-runs/run-7/events', async (route) => route.fulfill({ status: 200, contentType: 'text/event-stream', body: '' }))

  await page.goto('/projects')
  await page.getByTestId('enter-workspace').click()
  await page.waitForURL('**/projects/test/workspace/run-7')
  await expect(page.getByText('项目 Agent 配置')).toHaveCount(0)
})
