...
async function save() {
  ...
  try { const payload = { ...draft.value, id: undefined, status: 'DRAFT' as const, skills: JSON.parse(skillsJson.value), tools: JSON.parse(toolsJson.value), outputSchemaJson: JSON.parse(schemaJson.value), runtimeConfigJson: JSON.parse(runtimeJson.value) }; const saved = await saveAgentProfileVersion(profile.value.code, payload); profile.value.latestVersion = saved; editing.value = false; message.value = `草稿 v${saved.versionNo} 已保存` } catch (cause) { message.value = cause instanceof Error ? cause.message : '保存失败，请检查 JSON 格式' }
}
...
<template>
  <main class="agent-detail">
    ...
    <template v-if="profile">
      <header><div><span class="eyebrow">{{ profile.category }} · {{ profile.code }}</span><h1>{{ profile.name }}</h1><p>{{ profile.description }}</p></div><div class="actions"><button v-if="!editing" type="button" @click="beginEdit()">{{ profile.latestVersion?.status === 'PUBLISHED' ? '基于此创建草稿' : '编辑草稿' }}</button><button v-if="editing" type="button" class="primary" @click="save">保存草稿</button><button v-if="!editing && profile.latestVersion?.status === 'DRAFT'" type="button" class="primary" @click="publish">发布当前版本</button></div></header>
