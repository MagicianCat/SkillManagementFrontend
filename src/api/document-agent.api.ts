...
export type DocumentAgentJob = { jobKey: string; sessionKey: string; sequenceNo: number; status: string; runtimeJobId?: string | null; errorMessage?: string | null }
...
export async function sendDocumentAgentTurn(sessionKey: string, instruction: string, context: { sourceArtifactIds?: number[]; feishuDocuments?: { docId: string; docType: string; title?: string }[] } = {}) { return (await http.post<DocumentAgentJob>(`/document-agent/sessions/${sessionKey}/turns`, { instruction, ...context }, { headers: { 'Idempotency-Key': key() } })).data }
